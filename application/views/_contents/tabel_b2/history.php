<div class="row mb-2 align-items-center">
  <div class="col-md-9 d-flex align-items-center">
    <h1><?= $title ?> (ID = <?= $table_id ?>)<?= count_data($tbl_b2) ?><?= $phase ?></h1>
  </div>
  ...
  <table class="table table-light" id="data">
    ...
    <tbody>
      ...
      <?php foreach ($tbl_b2->result() as $tl_b2): ?>
        <tr>
          <td></td>
          <td><?= $tl_b2->id_history; ?></td>
          <td><?= $tl_b2->$tabel_b2_field1 ?></td>
          <td><?= $tl_b2->$tabel_b2_field2 ?></td>
          <td><?= $tl_b2->updated_at ?></td>
          <td><?= show_info_user($tl_b2->updated_by, $users) ?></td>
          <td>
            <?= btn_lihat($tl_b2->id_history) ?>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
...
<!-- modal lihat-->
<?php foreach ($tbl_b2->result() as $tl_b2): ?>
  <div id="lihat<?= $tl_b2->id_history; ?>" class="modal fade lihat" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <?= modal_header_id(lang('tabel_b2_alias'), $tl_b2->id_history) ?>
        ...
        <form>
          ...
          <div class="modal-footer">
            <?= btn_push('tabel_b2', $tl_b2->id_history) ?>
            <?= btn_tutup() ?>
          </div>
        </form>
