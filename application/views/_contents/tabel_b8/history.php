<div class="row mb-2 align-items-center">
  <div class="col-md-9 d-flex align-items-center">
    <h1><?= $title ?> (ID = <?= $table_id ?>)<?= count_data($tbl_b8) ?><?= $phase ?></h1>
  </div>
    ...
    <tbody>
      <?php foreach ($current->result() as $tl_cr): ?>
        <tr class="bg-light">
          <td></td>
          <td>[current]</td>
          <td><?= $tl_cr->$tabel_b8_field1 ?></td>
          <td><?= $tl_cr->$tabel_b8_field2 ?></td>
      ...
      <?php foreach ($tbl_b8->result() as $tl_b8): ?>
        <tr>
          <td></td>
          <td><?= $tl_b8->id_history; ?></td>
          <td><?= $tl_b8->$tabel_b8_field1 ?></td>
          <td><?= $tl_b8->$tabel_b8_field2 ?></td>
          <td><?= $tl_b8->updated_at ?></td>
          <td><?= show_info_user($tl_b8->updated_by, $users) ?></td>
          <td>
...
<!-- modal lihat-->
<?php foreach ($tbl_b8->result() as $tl_b8): ?>
  <div id="lihat<?= $tl_b8->id_history; ?>" class="modal fade lihat" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <?= modal_header_id(lang('tabel_b8_alias'), $tl_b8->id_history) ?>
        ...
        <form>
          ...
          <div class="modal-footer">
            <?= btn_push('tabel_b8', $tl_b8->id_history) ?>
            <?= btn_tutup() ?>
          </div>
        </form>
