<div class="row mb-2 align-items-center">
  <div class="col-md-9 d-flex align-items-center">
    <h1><?= $title ?> (ID = <?= $table_id ?>)<?= count_data($tbl_e3) ?><?= $phase ?></h1>
  </div>
  ...
  <table class="table table-light" id="data">
    ...
    <tbody>
      ...
      <?php foreach ($tbl_e3->result() as $tl_e3): ?>
        <tr>
          <td></td>
          <td><?= $tl_e3->id_history; ?></td>
          <td><?= $tl_e3->$tabel_e3_field1 ?></td>
          <td><?= $tl_e3->$tabel_e3_field2 ?></td>
          <td><?= $tl_e3->updated_at ?></td>
          <td><?= show_info_user($tl_e3->updated_by, $users) ?></td>
          <td>
...
<!-- modal lihat-->
<?php foreach ($tbl_e3->result() as $tl_e3): ?>
  <div id="lihat<?= $tl_e3->id_history; ?>" class="modal fade lihat" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <?= modal_header_id(lang('tabel_e3_alias'), $tl_e3->id_history) ?>
        ...
        <form>
          ...
          <div class="modal-footer">
            <?= btn_push('tabel_e3', $tl_e3->id_history) ?>
            <?= btn_tutup() ?>
          </div>
        </form>
