<div class="row mb-2 align-items-center">
  <div class="col-md-9 d-flex align-items-center">
    <h1><?= $title ?> (ID = <?= $table_id ?>)<?= count_data($tbl_e6) ?><?= $phase ?></h1>
  </div>
    ...
    <tbody>
      <?php foreach ($current->result() as $tl_cr): ?>
        <tr class="bg-light">
          <td></td>
          <td>[current]</td>
          <td><?= $tl_cr->$tabel_e6_field1 ?></td>
          <td><?= $tl_cr->$tabel_e6_field2 ?></td>
      ...
      <?php foreach ($tbl_e6->result() as $tl_e6): ?>
        <tr>
          <td></td>
          <td><?= $tl_e6->id_history; ?></td>
          <td><?= $tl_e6->$tabel_e6_field1 ?></td>
          <td><?= $tl_e6->$tabel_e6_field2 ?></td>
          <td><?= $tl_e6->updated_at ?></td>
          <td><?= show_info_user($tl_e6->updated_by, $users) ?></td>
          <td>
            <?= btn_lihat($tl_e6->id_history) ?>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
...
<!-- modal lihat-->
<?php foreach ($tbl_e6->result() as $tl_e6): ?>
  <div id="lihat<?= $tl_e6->id_history; ?>" class="modal fade lihat" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <?= modal_header_id(lang('tabel_e6_alias'), $tl_e6->id_history) ?>

        <!-- administrator tidak bisa melihat password user lain -->
        <form>
          <div class="modal-body">
            <?= table_data(
              row_data('tabel_e6_field1', $tl_e6->$tabel_e6_field1) . 
