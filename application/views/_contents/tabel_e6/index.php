...
<?php foreach ($tbl_e6->result() as $tl_e6) : ?>
  <img src='img/<?= $tabel_e6 ?>/<?= $tl_e6->$tabel_e6_field3 ?>' class="img-fluid rounded">
  <h2 class="pt-2"><?= $tl_e6->$tabel_e6_field2; ?> (Rp <?= number_format($tl_e6->$tabel_e6_field5, '2', ',', '.') ?> per hari)</h2>
  <ul class="list-unstyled ml-2">
    <li><?= $tabel_e1_alias ?> : </li>
    <?php foreach ($tbl_e1->result() as $tl_e1) : ?>
      <?php if ($tl_e6->$tabel_e6_field2 === $tl_e1->$tabel_e6_field2) { ?>
