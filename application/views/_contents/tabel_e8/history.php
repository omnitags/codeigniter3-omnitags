<div class="row mb-2 align-items-center">
  <div class="col-md-9 d-flex align-items-center">
    <h1><?= $title ?> (ID = <?= $table_id ?>)<?= count_data($tbl_e8) ?><?= $phase ?></h1>
  </div>
    ...
    <tbody>
      <?php foreach ($current->result() as $tl_cr): ?>
        <tr class="bg-light">
          <td></td>
          <td>[current]</td>
          <td><?= $tl_cr->$tabel_e8_field1 ?></td>
          <td><?= $tl_cr->$tabel_e8_field2 ?></td>
      ...
      <?php foreach ($tbl_e8->result() as $tl_e8): ?>
        <tr>
          <td></td>
          <td><?= $tl_e8->id_history; ?></td>
          <td><?= $tl_e8->$tabel_e8_field1 ?></td>
          <td><?= $tl_e8->$tabel_e8_field2 ?></td>
          <td><?= $tl_e8->updated_at ?></td>
          <td><?= show_info_user($tl_e8->updated_by, $users) ?></td>
          <td>
            <?= btn_lihat($tl_e8->id_history) ?>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
      ...
      <div class="modal-content">
        ...
        <form>
          ...
          <div class="modal-footer">
            <?= btn_push('tabel_e8', $tl_e8->id_history) ?>
            <?= btn_tutup() ?>
          </div>
        </form>
