...
<?php foreach ($tbl_e8->result() as $tl_e8) : ?>
  <img src='img/<?= $tabel_e8 ?>/<?= $tl_e8->$tabel_e8_field3 ?>' class="img-fluid rounded">
  <h2 class="pt-2"><?= $tl_e8->$tabel_e8_field2; ?> (Rp <?= number_format($tl_e8->$tabel_e8_field5, '2', ',', '.') ?> per hari)</h2>
  <ul class="list-unstyled ml-2">
    <li><?= $tabel_e1_alias ?> : </li>
    <?php foreach ($tbl_e1->result() as $tl_e1) : ?>
      <?php if ($tl_e8->$tabel_e8_field2 === $tl_e1->$tabel_e8_field2) { ?>
