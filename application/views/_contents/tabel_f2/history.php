    ...
    <tbody>
      <?php foreach ($current->result() as $tl_cr): ?>
        <tr class="bg-light">
          <td></td>
          <td>[current]</td>
          <td><?= $tl_cr->$tabel_f2_field1 ?></td>
          <td><?= $tl_cr->$tabel_f2_field2 ?></td>
      ...
      <?php foreach ($tbl_f2->result() as $tl_f2): ?>
        <tr>
          ...
          <td><?= show_info_user($tl_f2->updated_by, $users) ?></td>
          <td>
  ...
  <div id="lihat<?= $tl_f2->id_history; ?>" class="modal fade lihat" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <?= modal_header_id($tabel_f2_alias, $tl_f2->id_history) ?>
        ...
        <form>
          ...
          <div class="modal-footer">
            <?= btn_push('tabel_f2', $tl_f2->id_history) ?>
            <?= btn_tutup() ?>
          </div>
        </form>
