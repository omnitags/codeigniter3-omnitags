<div class="row mb-2 align-items-center">
  <div class="col-md-9 d-flex align-items-center">
    <h1><?= $title ?> (ID = <?= $table_id ?>)<?= count_data($tbl_f3) ?><?= $phase ?></h1>
  </div>
    ...
    <tbody>
      <?php foreach ($current->result() as $tl_cr): ?>
        <tr class="bg-light">
          <td></td>
          <td>[current]</td>
          <td><?= $tl_cr->$tabel_f3_field1 ?></td>
          <td><?= $tl_cr->$tabel_f3_field2 ?></td>
      ...
      <?php foreach ($tbl_f3->result() as $tl_f3): ?>
        <tr>
          <td></td>
          <td><?= $tl_f3->id_history; ?></td>
          <td><?= $tl_f3->$tabel_f3_field1 ?></td>
          <td><?= $tl_f3->$tabel_f3_field2 ?></td>
          <td><?= $tl_f3->updated_at ?></td>
          <td><?= show_info_user($tl_f3->updated_by, $users) ?></td>
          <td>
            <?= btn_lihat($tl_f3->id_history) ?>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
...
<!-- modal lihat-->
<?php foreach ($tbl_f3->result() as $tl_f3): ?>
  <div id="lihat<?= $tl_f3->id_history; ?>" class="modal fade lihat" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <?= modal_header_id(lang('tabel_f3_alias'), $tl_f3->id_history) ?>
        ...
        <form>
          ...
          <div class="modal-footer">
            <?= btn_push('tabel_f3', $tl_f3->id_history) ?>
            <?= btn_tutup() ?>
          </div>
        </form>
